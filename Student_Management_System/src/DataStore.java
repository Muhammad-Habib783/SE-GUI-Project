 ...
 * students.csv -> id,name,contact,klass,password,feePaid,attendanceEncoded,marksEncoded
...
public class DataStore 
{
    ...
    public static class Student 
    {
        ...
        public String klass; // class name
        public String password;
        public double feePaid = 0.0;
        public Map<String,String> attendance = new LinkedHashMap<>(); // date -> P/A
        public Map<String,Integer> marks = new LinkedHashMap<>(); // subj -> mark
        public Student(String id,String name,String contact,String klass,String password){
            this.id=id; this.name=name; this.contact=contact; this.klass=klass; this.password=password;
        }
        public double getAttendancePercentage()
        {
            if(attendance.isEmpty()) return 0.0;
            int present=0;
            for(String v: attendance.values()) if("P".equals(v)) present++;
            return present*100.0/attendance.size();
        }
        public int getPresentCount()
        {
             int c=0; for(String v:attendance.values()) if("P".equals(v)) c++; return c; 
            }
    ...
    public static class Teacher 
    {
        ...
        public Teacher(String id,String name,String contact,String password){
            this.id=id; this.name=name; this.contact=contact; this.password=password;
        }
        public int totalAssignedSubjects(){
            int c=0; for(List<String> l: assignedClassSubjects.values()) c += l.size(); return c;
        }
    }
    ...
    {
        ensureFolder();
        classes.clear(); students.clear(); teachers.clear();
        loadClasses(); loadStudents(); loadTeachers();
        // rebuild class student lists
        for(ClassRoom cr : classes) cr.studentIds.clear();
        for(Student s : students){
            ClassRoom c = getClassByName(s.klass);
            if(c != null && !c.studentIds.contains(s.id)) c.studentIds.add(s.id);
        }
    }
    ...
    private static String[] splitCsv(String line, int minCols)
    {
        ...
        for(int i=0;i<line.length();i++)
            ...
            else if(ch==',' && !inQuotes){ out.add(sb.toString()); sb.setLength(0); 
            ...
            while((line=br.readLine())!=null)
                ...
                if(cols.length>1){
                    ...
                    if(!subj.isEmpty())
                        ...
                        for(String s: ss) if(!s.trim().isEmpty()) cr.subjects.add(s.trim());
                    }
                }
                if(cols.length>2)
                    {
 ...
 private static void loadStudents()
        ...
        try(BufferedReader br=new BufferedReader(new FileReader(f))){
            ...
            while((line=br.readLine())!=null){
                ...
                if(cols.length>5 && !unquote(cols[5]).isEmpty()){
                    try{ s.feePaid = Double.parseDouble(unquote(cols[5])); }catch(Exception ex){ s.feePaid=0.0; }
                }
                if(cols.length>6 && !unquote(cols[6]).isEmpty()){
                    ...
                    for(String p: pairs){
                        ...
                        String[] kv = p.split("~",2);
                        if(kv.length==2) s.attendance.put(kv[0], kv[1]);
                    }
                }
                if(cols.length>7 && !unquote(cols[7]).isEmpty()){
                    ...
                    for(String p: pairs){
                        ...
                        String[] kv = p.split(":",2);
                        if(kv.length==2){
                            try{ s.marks.put(kv[0], Integer.parseInt(kv[1])); }catch(Exception ex){}
                        }
                    }
                }
                students.add(s);
            ...
            while((line=br.readLine())!=null){
                ...
                if(cols.length>4 && !unquote(cols[4]).isEmpty()){
                    ...
                    for(String cp: classParts){
                        if(cp.trim().isEmpty()) continue;
                        String[] kv = cp.split("~",2);
                        if(kv.length==2){
                            String cname = kv[0];
                            String[] subs = kv[1].split("\\|");
                            List<String> list = new ArrayList<>();
                            for(String s: subs) if(!s.trim().isEmpty()) list.add(s.trim());
    ...
    public static void saveData(){
        saveClasses(); saveStudents(); saveTeachers();
    }
        ...
        try(PrintWriter pw = new PrintWriter(new FileWriter(STUDENTS_FILE))){
            for(Student s: students){
                ...
                for(Map.Entry<String,String> e : s.attendance.entrySet()){
                    if(!first) att.append("^"); first=false;
                    att.append(e.getKey()).append("~").append(e.getValue());
                }
                StringBuilder mk = new StringBuilder();
                first=true;
                for(Map.Entry<String,Integer> e : s.marks.entrySet()){
                    if(!first) mk.append("^"); first=false;
                    mk.append(e.getKey()).append(":").append(e.getValue());
                }
                pw.println(escapeCsv(s.id)+","+escapeCsv(s.name)+","+escapeCsv(s.contact)+","+escapeCsv(s.klass)+","+escapeCsv(s.password)+","+escapeCsv(String.valueOf(s.feePaid))+","+escapeCsv(att.toString())+","+escapeCsv(mk.toString()));
            }
        }catch(Exception e){ e.printStackTrace(); }
    }
        ...
        try(PrintWriter pw = new PrintWriter(new FileWriter(TEACHERS_FILE))){
            for(Teacher t: teachers){
                ...
                for(Map.Entry<String,List<String>> en : t.assignedClassSubjects.entrySet()){
                    if(!firstClass) enc.append(";");
                    firstClass=false;
                    enc.append(en.getKey()).append("~").append(String.join("|", en.getValue()));
                }
                pw.println(escapeCsv(t.id)+","+escapeCsv(t.name)+","+escapeCsv(t.contact)+","+escapeCsv(t.password)+","+escapeCsv(enc.toString()));
            }
        }catch(Exception e){ e.printStackTrace(); }
    }
    ...
    public static ClassRoom getClassByName(String name){
        ...
        for(ClassRoom c: classes) if(name.equals(c.name)) return c;
        return null;
    }
    public static Student getStudentById(String id){
        if(id==null) return null;
        for(Student s: students) if(id.equals(s.id)) return s;
        return null;
    }
    public static Teacher getTeacherById(String id){
        if(id==null) return null;
        for(Teacher t: teachers) if(id.equals(t.id)) return t;
        return null;
    }
    public static boolean studentIdExists(String id){ return getStudentById(id) != null; }
    public static boolean teacherIdExists(String id){ return getTeacherById(id) != null; }

    public static String todayDate(){ return new SimpleDateFormat("yyyy-MM-dd").format(new Date()); }
    ...
    public static boolean isSubjectAssignedToAnyTeacher(String className, String subject){
        for(Teacher t: teachers){
            ...
            if(list!=null && list.contains(subject)) return true;
        }
        return false;
    }
    ...
    public static boolean assignSubjectsToTeacher(String teacherId, String className, List<String> subjectsOut){
        ...
        for(String s: subjectsOut){
            if(isSubjectAssignedToAnyTeacher(className,s)) {
                ...
                if(cur==null || !cur.contains(s)) return false;
            }
        }
    ...
    public static boolean updateTeacherAssignedSubjects(String teacherId, String className, List<String> newSubjects){
        ...
        for(Teacher other : teachers){
            if(other.id.equals(teacherId)) continue;
            List<String> list = other.assignedClassSubjects.get(className);
            if(list!=null){
                for(String s: newSubjects) if(list.contains(s)) return false;
            }
        }
        if(newSubjects.isEmpty()) t.assignedClassSubjects.remove(className);
        else t.assignedClassSubjects.put(className, new ArrayList<>(newSubjects));
        saveData();
        return true;
    }
    ...
    public static boolean addStudent(Student s){
        ...
        if(studentIdExists(s.id)) return false;
        students.add(s);
        ClassRoom c = getClassByName(s.klass);
        if(c!=null && !c.studentIds.contains(s.id)) c.studentIds.add(s.id);
        saveData();
    ...
    public static boolean updateStudent(String oldId, Student newS){
        ...
        if(!oldId.equals(newS.id)){
            if(studentIdExists(newS.id)) return false;
        }
        ...
        if(!existing.klass.equals(newS.klass)){
            ...
            if(from!=null) from.studentIds.remove(existing.id);
            if(to!=null && !to.studentIds.contains(newS.id)) to.studentIds.add(newS.id);
        } else {
            ...
            if(cr!=null){
                ...
                if(!cr.studentIds.contains(newS.id)) cr.studentIds.add(newS.id);
            }
        }
        // replace fields
        existing.id = newS.id;
    ...
    public static boolean deleteStudent(String id){
        ...
        if(cr!=null) cr.studentIds.remove(id);
        // remove from students list
        Iterator<Student> it = students.iterator();
        while(it.hasNext()){
            Student ss = it.next();
            if(ss.id.equals(id)){ it.remove(); break; }
        }
        saveData();
        return true;
    }
    ...
    public static boolean addTeacher(Teacher t){
        if(t==null) return false;
        if(teacherIdExists(t.id)) return false;
        teachers.add(t);
        saveData();
        return true;
    }
    public static boolean updateTeacher(String oldId, Teacher newT){
        ...
        if(!oldId.equals(newT.id)){
            if(teacherIdExists(newT.id)) return false;
        }
        existing.id = newT.id;
    ...
    public static boolean deleteTeacher(String id){
        ...
        if(t==null) return false;
        // remove assigned references only exist in teacher object; no cross links needed
        Iterator<Teacher> it = teachers.iterator();
        while(it.hasNext()){
            Teacher tt = it.next();
            if(tt.id.equals(id)){ it.remove(); break; }
        }
        saveData();
        return true;
    }
    ...
    public static boolean addSubjectToClass(String className, String subject){
        ...
        if(c.subjects.contains(subject)) return false;
    ...
    public static boolean updateSubjectInClass(String className, String oldSubject, String newSubject){
        ClassRoom c = getClassByName(className);
        if(c==null) return false;
        if(!c.subjects.contains(oldSubject)) return false;
        newSubject = newSubject.trim();
        if(newSubject.isEmpty()) return false;
        // do not allow duplicate
        if(c.subjects.contains(newSubject) && !oldSubject.equals(newSubject)) return false;
            ...
            for(Map.Entry<String,List<String>> en : t.assignedClassSubjects.entrySet()){
                if(en.getKey().equals(className)){
                    ...
                    for(int i=0;i<list.size();i++){
                        if(list.get(i).equals(oldSubject)) list.set(i, newSubject);
                    }
                }
            }
        }
    ...
    public static boolean deleteSubjectFromClass(String className, String subject){
        ...
        for(Student s: students) s.marks.remove(subject);
        // remove assignments from teachers
        for(Teacher t: teachers){
            List<String> list = t.assignedClassSubjects.get(className);
            if(list!=null){
                list.remove(subject);
                if(list.isEmpty()) t.assignedClassSubjects.remove(className);
            }
        }
        saveData();
        return true;
    ...
    public static boolean markAttendanceForStudent(String studentId, String date, String value){
        Student s = getStudentById(studentId);
        if(s==null) return false;
        if(!"P".equals(value) && !"A".equals(value)) return false;
    ...
    // Helper: remove all previous data (clear files) - per your request "remove previous data"
    // NOTE: Use with caution. We'll provide method but not call it automatically.
    public static void clearAllDataFiles(){
        try{ new File(CLASSES_FILE).delete(); new File(STUDENTS_FILE).delete(); new File(TEACHERS_FILE).delete(); }
        catch(Exception e){ e.printStackTrace(); }
    }
    public static boolean updateTeacherAssignedSubjectsAdvanced(String teacherId, String oldSubject, String newClass, String newSubject){
    Teacher t = getTeacherById(teacherId);
    if(t == null) return false;
    ...
    for(Map.Entry<String,List<String>> en : t.assignedClassSubjects.entrySet()){
        if(en.getValue().contains(oldSubject)){
            ...
            if(en.getValue().isEmpty()) t.assignedClassSubjects.remove(en.getKey());
            break;
        }
    }

    // Check exclusivity: new class-subject not assigned to others
    if(isSubjectAssignedToAnyTeacher(newClass, newSubject)) return false;
